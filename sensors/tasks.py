...
def aggregate_1sec_data():
    ...
    # Process each sensor that has data in this window
    sensor_ids = SensorReading.objects.filter(
...
def aggregate_1min_data():
    ...
    # Process each sensor that has data in this window
    sensor_ids = SensorAggregated1Sec.objects.filter(
        timestamp__gte=start_time, timestamp__lt=end_time
    ).values_list('sensor_id', flat=True).distinct()
...
def aggregate_1hour_data():
    ...
    # Process each sensor that has data in this window
    sensor_ids = SensorAggregated1Min.objects.filter(
        timestamp__gte=start_time, timestamp__lt=end_time
    ).values_list('sensor_id', flat=True).distinct()
...
def check_sensor_dropouts():
    ...
    # Check all sensors that have ever sent data
